...
vstruct_dot(#vstruct{tree = Tree}) ->
    ...
    io:fwrite("edge [dir=back, arrowsize=.7];~n"),
    io:fwrite("node [shape=circle, fontname=\"Tt-Kp\"];~n"),
    io:fwrite("ranksep=0.3;\n"),
    io:fwrite("nodesep=0.2;\n"),
...
vstruct_dot_rec(Path, #vstruct_p{votes = _V, id = Id}) ->
    % io:format("\"~p\" [label=\"~s | ~p\"];~n", [Path, atom_to_list(Id), V]);
    io:format("\"~p\" [label=\"~s\"];~n", [Path, string:to_upper(atom_to_list(Id))]);
    %% io:format("indices:\"~p\" -> \"~p\";~n", [Id, Path]);
vstruct_dot_rec(Path, #vstruct_v{votes = _V, thresh = T, children = Children}) ->
    % io:format("\"~p\" [label=\"V | { ~p | ~p }\"];~n", [Path, V, T]),
    io:format("\"~p\" [label=\"V | ~p\", shape=Mrecord, fontname=\"Kp\"];~n",
              [Path, T]),

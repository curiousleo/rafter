...
\begin{description}
    \item[\enquote{all processes from a complete column}] For each column in the grid (each consisting of \(r\) processes), we create a substructure whose virtual root node has threshold \(r\). These subroots are then given a common virtual parent node, the \textsc{cc}-cover root (for complete column cover). Since we require only one complete column, that parent node's threshold is set to one.
    \item[\enquote{one process from each column}] Again we create a substructure for each column in the grid. Their virtual root nodes have a threshold of one (since we require \emph{one} process from each column), and are again given a common virtual parent node, called the \textsc{c}-cover root (for column cover). As one node from each column is needed, its threshold must be \(c\).
\end{description}
...
The Grid Protocol specification demands that both requirements must be fulfilled in order to reach a quorum. We thus make the \textsc{cc}-cover root and the \textsc{c}-cover root the children of a virtual node with a threshold of two as in \autoref{fig:grid4-covers}. This makes sure that an election only succeeds when both a \textsc{cc}-cover and a \textsc{c}-cover have been found.

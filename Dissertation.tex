...
\begin{lstlisting}[style=erlang, caption={Majority Protocol voting structure generator code in Erlang}, label=lst:majority, float=htpb]
-record(vstruct_p, { votes = 1 :: pos_integer(), id :: peer() }).
-record(vstruct_v, { votes = 1 :: pos_integer(), thresh :: pos_integer(),
          children :: [ #vstruct_v{} | #vstruct_p{} ] }).
-record(vstruct, { tree :: #vstruct_v{}, indices :: dict() }).
...
\begin{lstlisting}[style=cpp, caption={Majority Protocol voting structure generator code in C++, for comparison}, label=lst:majority, float=htpb]
typedef int id;
typedef std::vector<std::size_t> path_t;
typedef std::vector<path_t> paths_t;
typedef std::map<id,std::vector<std::vector<std::size_t> > > indices_t;
typedef std::pair<id, std::vector<std::vector<std::size_t> > > value_t;
typedef struct { int id; int votes; } vstruct_p;
typedef struct { std::size_t thresh; void *children; } vstruct_v;
typedef struct { vstruct_v *tree; indices_t *indices; } vstruct;

vstruct majority(std::vector<id> ids) {
    std::size_t ids_len = ids.size();
    std::size_t threshold = (std::size_t)std::round((ids_len + 1.0) / 2.0);
    std::vector<vstruct_p> *nodes = new std::vector<vstruct_p>(ids_len);

    for(std::vector<id>::iterator ids_it = ids.begin();
            ids_it != ids.end(); ++ids_it) {
        nodes->push_back((vstruct_p){ .id = *ids_it, .votes = 1 });
    }
    ...
    for(std::vector<id>::iterator ids_it = ids.begin();
            ids_it != ids.end(); ++ids_it) {
        paths_t paths = paths_t { path_t {k} };
        value_t id_paths_pair(*ids_it, paths);
        indices->insert(id_paths_pair);
    }

    vstruct_v *tree = new vstruct_v(
            (vstruct_v){ .thresh = threshold, .children = nodes});
    return (vstruct){ .tree = tree, .indices = indices};
}
\end{lstlisting}

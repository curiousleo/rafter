...
\DeclareFloatVCode{rulebelow}%
    {{\color{gray}\par\vspace{-18pt}\rule\hsize{0.5pt}\vskip4pt\par}}
\floatsetup[listing]{
    style=plain,
    frameset={\fboxsep6pt}
    capposition=bottom,
...
To simplify this translation, I will now introduce two common patterns that will be used over and over in the remainder of this section: \emph{any}- and \emph{all}-nodes.\footnote{Note that the terms any-node and all-node are not used in structured voting literature, but I found them very helpful when designing or reading voting structures.}
...
In the following, \(n\) stands for the number of processes.
...
The Grid Protocol specification demands that both requirements must be fulfilled in order to reach a quorum. We thus make the \textsc{cc}-cover root and the \textsc{c}-cover root the children of an all-node as in \autoref{fig:grid4-covers}. This makes sure that an election only succeeds when both a \textsc{cc}-cover and a \textsc{c}-cover have been found.

Going the other way round, that is, \emph{reading} \autoref{fig:grid4-covers}, we can regard the tree-shaped voting structure as the abstract syntax tree of a logical expression. We can write it down as follows, where \(c\) ranges over the columns and \(p\) over the processes' votes:

\begin{align*}
    \text{Success} &\equiv \text{\textsc{c}-cover} \wedge \text{\textsc{cc}-cover} \\
    &= \left[ \bigwedge_c \bigvee_{p\,\in\,c} p \right]
       \wedge \left[ \bigvee_c \bigwedge_{p\,\in\,c} p \right] \\
    % &= (\text{any process from column 1} \wedge \text{any process from column 2}) \\
    % &\quad\wedge (\text{all processes from column 1} \vee \text{all processes from column 2}) \\
    &= [(A \vee C) \wedge (B \vee D)] \wedge [(A \wedge C) \vee (B \wedge D)]
    \label{mat:grid4-bool}
\end{align*}
...
Take the tree in the top left-hand corner of \autoref{fig:grid3_13} as an example. Building the voting structure bottom-up, we build a substructure representing the subtree with root \texttt{B}. This substructure should only vote if \texttt{B} and one of \texttt{C}, \texttt{D} or \texttt{E} have voted.

Since it does not matter which of \texttt{C}, \texttt{D} and \texttt{E} voted, we can make all three children of a virtual node with a threshold of one. This virtual node in some sense represents the logical \emph{or} since it votes as soon as any of its children do.

To complete the substructure, we make the virtual \emph{or}-node and \texttt{B} the children of a virtual node with threshold set to two (a logical \emph{and} in some sense.) This \emph{and}-node is the root of the substructure containing the four processes, and its root, by construction, votes only if \texttt{B} and one of \texttt{C}, \texttt{D} or \texttt{E} have voted. The entire voting structure is then built up recursively in this way.

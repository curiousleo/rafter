...
\begin{table}[h]
    ...
    \begin{tabularx}{\textwidth}{X c c S c c}
        ...
        \midrule
        {\fontfamily{jkpxkosn}\selectfont t1.micro } & up to 2 & 1 & 0.613 & \textsc{ebs} only & very low \\
        {\fontfamily{jkpxkosn}\selectfont m1.xlarge } & 8 & 4 & 15 & 4 \(\times\) 420 & high \\
        \bottomrule
    \end{tabularx}
    }
...
\begin{figure}[h]
    \centering
    \includegraphics[width=\textwidth, keepaspectratio]{Images/ttc_plain-maj.pdf}
    \caption[\textsc{ttc} for original Rafter and Majority Voting]{Time to consensus measurements for original Rafter (\enquote{plain}) and the Majority Protocol developed as part of this project (\enquote{majority}).}
    \label{fig:plain-maj-ttc}
\end{figure}

\begin{figure}[h]
    \centering
    \includegraphics[width=\textwidth, keepaspectratio]{Images/ttc_maj-tree-grid.pdf}
    \caption[\textsc{ttc} for different structured voting schemes]{Time to consensus measurements for the Majority Protocol (\enquote{majority}), the Tree Quorum protocol (\enquote{tree2} and \enquote{tree3} with branching factors \(d = 2\) and \(d = 3\), respectively), and the Grid Protocol (\enquote{grid}).}
    \label{fig:plain-maj-ttc}
\end{figure}

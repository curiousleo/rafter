...
Then \texttt{benchmark} task uses these helper functions as follows: For each combination of cluster size, protocol and failure mode that is to be tested, the procedure starts by deploying the latest version of the project source code to all instances as required and compiling it (using the \texttt{deploy} task.) It then starts up an Erlang node with a fresh name on each of them. These nodes start in the \texttt{follower} state and with an empty initial configuration.

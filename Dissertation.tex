...
The choice of Erlang and Rafter then dictated my primary programming toolkit: The Erlang language, its core libraries (called the Open Telephony Platform, or \textsc{otp}), the static type checker Dialyzer\footnote{Dialyzer, the Discrepancy Analyzer for Erlang programs (\url{http://www.erlang.org/doc/man/dialyzer.html}), can be used to statically type check Erlang programs.}, and the unit testing framework QuickCheck\footnote{QuickCheck (\url{http://www.quviq.com/index.html}) is a commercial unit testing framework by Quvik \textsc{se}, which is used in Rafter.}

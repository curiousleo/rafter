...
\centerline{\large Computer Science Part II Project Proposal} \vspace{0.4in}
\centerline{\Large\bf Extending Raft with Structured Voting} \vspace{0.3in}
\centerline{\large \emph{October 21, 2013}}
...
\item Design and implementation of a benchmark suite for a distributed key-value store on top of Rafter. The benchmark metrics must include latency and throughput measured under a range of system loads.

\item The project must be set up so it can be run as a distributed key-value store in the Amazon Elastic Compute Cloud (EC2) infrastructure. Then the benchmarks have to be run and the results collected and evaluated. The evaluation must include a comparison of the benchmarks collected for the different voting schemes that have been implemented.
...
\item[TCP or HTTP interface.] Extend the project with a CRUD (Create, Read, Update, Delete) API that can be accessed via TCP or HTTP.
...
\emph{Milestones:} Phase 1 Report Form on Monday 14, 2013, then a Project Proposal complete with as realistic a timetable as possible by Wednesday 17, 2013, approval from Overseers and confirmed availability of any special resources needed. Signatures from Supervisor and Director of Studies.
...
\emph{Milestones:} Running the benchmarking suite locally and collecting and interpreting the results.

\subsection{Weeks 17 and 18 (Feb 10 to Feb 23)%
  \label{weeks-17-and-18-feb-10-to-feb-23}%
}

Set-up and configuration for Amazon EC2. Running the benchmarking suite.
